...
def generar_informe_corporativo(encargado, df_filtrado, fecha_ini, fecha_fin):
    ...
    # 1. Encabezado con Logo (Si existe logo.png)
    try:
        # Intenta cargar logo.png del directorio raiz
        doc.add_picture('logo.png', width=Inches(1.5)) 
    except:
        pass # Si no hay logo, sigue sin error
    ...
    # 2. Objetivo Corporativo
    doc.add_heading('1. Objetivo del Reporte', level=1)
    p = doc.add_paragraph(
        "El presente documento tiene como finalidad certificar la correspondencia entre los registros de ingreso y salida de combustible, "
        "validando la integridad de los datos reportados por el proveedor frente a la gestión operativa interna. "
        "Asimismo, se busca identificar desviaciones en el rendimiento de la flota que puedan impactar en la eficiencia operativa de Ekos Forestal S.A."
    )
    p.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY

    # 3. Análisis de Rendimiento (Lógica Natural)
    ...
    if 'tipo_operacion' in df_filtrado.columns:
        ...
        for cod in unique_maqs:
            ...
            # Calcular L/ajustados para promedio
            if len(dm) > 1:
            ...
            if cod in FLOTA:
                ...
                if unidad == 'KM':
                    ...
                    if rend < ideal * (1 - MARGEN_TOLERANCIA):
                        maquinas_alerta.append((cod, rend, ideal, unidad, "bajo"))
                else: # Horas
                    rend = l_ajustados / rec if rec > 0 else 0
                    if rend > ideal * (1 + MARGEN_TOLERANCIA):
                        maquinas_alerta.append((cod, rend, ideal, unidad, "alto"))

    # Escribir texto natural sobre alertas
    if maquinas_alerta:
        ...
        for maq, real, ideal, un, tipo in maquinas_alerta:
            ...
            if un == 'KM':
                txt = f"• La unidad {maq} presentó un rendimiento de {real:.2f} Km/L, situándose por debajo del estándar ideal de {ideal} Km/L. Esto representa una desviación del {diff_pct:.1f}%, lo cual sugiere un consumo elevado para el recorrido realizado."
            else:
                txt = f"• El equipo {maq} registró un consumo horario de {real:.2f} L/H, excediendo el parámetro esperado de {ideal} L/H. Esta desviación del {diff_pct:.1f}% podría indicar condiciones de operación exigentes o la necesidad de una revisión técnica preventiva."
            
            p = doc.add_paragraph(txt)
            p.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
    else:
        doc.add_paragraph("Tras el análisis de los registros del período, no se observaron desviaciones significativas en el rendimiento de las máquinas. La flota opera dentro de los parámetros de tolerancia establecidos.")
    ...
    hdr_cells[0].text = 'Código Máquina'
    hdr_cells[1].text = 'Litros Totales'
    hdr_cells[2].text = 'Recorrido Total'
    
    if 'tipo_operacion' in df_filtrado.columns:
        resumen = df_maq.groupby('codigo_maquina').agg({'litros': 'sum'}).reset_index()
        for index, row in resumen.iterrows():
            row_cells = table.add_row().cells
            row_cells[0].text = str(row['codigo_maquina'])
            row_cells[1].text = f"{row['litros']:.1f}"
    ...
    doc.add_paragraph(
        "Se recomienda mantener un monitoreo constante sobre las unidades listadas en la sección de hallazgos. "
        "Es vital asegurar que todos los registros de carga incluyan la diferenciación correcta entre Nafta y Diésel para evitar inconsistencias en el cruce de datos con el proveedor Petrobras. "
        "Asimismo, se sugiere validar con los operadores las condiciones de trabajo en los días donde se presentaron los picos de consumo."
    )

    # Footer profesional
    doc.add_paragraph("\n")
    footer = doc.add_paragraph("Informe generado automáticamente por el sistema Ekos Control.")
